    ...
    if (app.appleId != null) {
      return app.appleLanguagesToCheck.flatMap { language ->
        (1..app.appleNumberOfPagesToCheck)
          .flatMap { pageNumber ->
            val response = httpClient.get("$language/rss/customerreviews/page=$pageNumber/id=${app.appleId}/sortby=mostrecent/xml?urlDesc=/customerreviews/id=${app.appleId}/xml")

            if (response.status.isSuccess()) {
              val newDocumentBuilder = DocumentBuilderFactory.newInstance().newDocumentBuilder()
              val document = newDocumentBuilder.parse(InputSource(StringReader(response.bodyAsText())))
